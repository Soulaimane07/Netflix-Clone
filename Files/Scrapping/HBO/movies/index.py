...
def ScrappingData(url):
    ...
    if response.status_code == 200:
        ...
        movie_items = soup.find_all('div', class_='container-fluid')
        
        i = 0
        for container in movie_items:
            movie_itemss = container.find_all('div', class_='row')
            
            for movie in movie_itemss:
                    title_element = movie.find('div', class_='card-title')
                    title = title_element.text.strip() if title_element else 'No Title'
                    
                    cardimage = movie.find('img', class_='img')
                    cardimage_src = cardimage.get('src') if cardimage else None

                    details_element = movie.find('a', class_='card-button usePointer')
                    details_url = details_element.get('href') if details_element else None

                    if details_url: 
                        print(i)
                        print(f'title: {title}')
                        print(f'image: {cardimage_src}')
                        print(f'src: {details_url}')
                        print()
...
if __name__ == "__main__":
    url = 'https://www.hbo.com/movies/a-z'
    ScrappingData(url)